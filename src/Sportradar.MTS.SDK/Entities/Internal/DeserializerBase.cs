...
namespace Sportradar.MTS.SDK.Entities.Internal
{
    ...
    {
        private static readonly ILog Logger = SdkLoggerFactory.GetLogger(typeof(DeserializerBase));
        ...
        private static Dictionary<string, SerializerWithInfo> InitDeserializer()
        {
            ...
            foreach (var baseType in BaseTypes)
            {
                foreach (var feedMessagesType in baseType.Assembly.GetTypes().Where(t => t != baseType && baseType.IsAssignableFrom(t)))
                {
                    ...
                    {
                        Logger.Error($"Type {feedMessagesType.FullName} cannot be deserialized with {typeof(Deserializer<>).FullName} because the name of RootXmlElement is not specified");
                        continue;
                    }
                    if (serializers.ContainsKey(rootElementName))
                    {
                        Logger.Warn($"Deserializer associated with name {rootElementName} already exists");
                        continue;
                    }
