...
namespace Sportradar.MTS.SDK.Entities.Internal.Cache
{
    ...
    {
        private static readonly ILog Log = SdkLoggerFactory.GetLogger(typeof(MarketDescriptionCacheItem));
        ...
        private void SaveOutcomes(long marketId, ICollection<OutcomeDescriptionDTO> outcomes, CultureInfo culture)
        {
            foreach (var outcomeDto in outcomes)
            {
                ...
                {
                    Log.Warn($"Could not merge outcome[Id={outcomeDto.Id}] on marketDescription[Id={marketId}] because the specified outcome does not exist on stored market description");
                }
            }
        }
        ...
        private void SaveMappings(long marketId, ICollection<MarketMappingDTO> mappings)
        {
            foreach (var mappingDto in mappings)
            {
                ...
                {
                    Log.Warn($"Could not merge mapping[MarketId={mappingDto.MarketTypeId}:{mappingDto.MarketSubTypeId}] on marketDescription[Id={marketId}] because the specified mapping" +
