...
namespace Sportradar.MTS.SDK.Test.Api
{
    ...
    public class MtsAuthServiceTests
    {
        ...
        public void Init()
        {
            ...
            _mtsAuthService = new MtsAuthService(_dataProvider, config);
        }
        ...
        public void DeserializeTokenMessageTest()
        {
            ...
            using (var stream = File.OpenText(tokenPath))
            {
                var tokenData = stream.ReadToEnd();
                Assert.IsTrue(!string.IsNullOrEmpty(tokenData));
                var tokenDto = new JsonDeserializer<AccessTokenDTO>().Deserialize<AccessTokenDTO>(tokenData);
